...
export async function POST(request: NextRequest) {
  try {
    ...
    // Récupérer la revendication pour obtenir le provider_id et user_id
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Erreur lors de l\'approbation de la revendication: ' + updateError.message },
        { status: 500 }
      )
    }
    ...
    // Mettre à jour le provider pour l'associer à l'utilisateur
    ...
    if (providerUpdateError) {
      console.error('Error updating provider:', providerUpdateError)
      // Ne pas échouer complètement si la mise à jour du provider échoue
      // La revendication a été approuvée, c'est l'essentiel
    }
